...
const getSummaryTypeBadgeClass = (summaryType: string) => {
  switch (summaryType) {
    case 'DAILY':
...
const parseBoldSegments = (content: string, isModel: boolean) => {
  ...
  for (let j = 0; j < parts.length; j++) {
    ...
    if (j % 2 === 1) {
      elements.push(
        <strong key={`bold-${j}`} className={cn("font-extrabold", isModel ? "text-zinc-950 dark:text-white" : "text-white dark:text-zinc-900")}>
          {part}
        </strong>
      );
    } else {
...
const renderSingleLine = (line: string, index: number, isModel: boolean) => {
  const trimmed = line.trim();
  const isBullet = trimmed.startsWith('-') || trimmed.startsWith('*');
  const content = isBullet ? trimmed.replace(/^[-*]\s+/, '') : trimmed;
  const elements = parseBoldSegments(content, isModel);

  if (isBullet) {
    return (
      <li key={`bullet-${index}`} className={cn("list-disc ml-4 my-1", isModel ? "text-zinc-755 dark:text-zinc-300" : "text-white dark:text-zinc-900")}>
        <span>{elements}</span>
      </li>
    );
  }
  return (
    <p key={`p-${index}`} className={cn("my-1 leading-normal min-h-[0.75rem]", isModel ? "text-zinc-755 dark:text-zinc-300" : "text-white/95 dark:text-zinc-900")}>
      {elements}
    </p>
  );
};
...
const renderMarkdown = (text: string, isModel: boolean) => {
  if (!text) return null;
  return text.split('\n').map((line, index) => renderSingleLine(line, index, isModel));
};
...
const GenerateSummaryModal = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <button type="button" onClick={() => !isSubmitting && onClose()} className="absolute inset-0 bg-zinc-900/60 dark:bg-black/80 backdrop-blur-sm animate-in fade-in duration-200 border-0 cursor-default" aria-label="Close modal" />
        ...
        {isSubmitting ? (
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div className="space-y-1.5">
              <span className="block text-[9px] font-bold text-zinc-400 uppercase tracking-widest px-1">Report Cadence</span>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <span className="block text-[9px] font-bold text-zinc-400 uppercase tracking-widest px-1">Period Start</span>
                <MyanmarDateInput
                  required
              ...
              <div className="space-y-1.5">
                <span className="block text-[9px] font-bold text-zinc-400 uppercase tracking-widest px-1">Period End</span>
                <MyanmarDateInput
                  required
                  value={periodEndDate}
...
const DetailedSummaryModal = ({
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
      <button type="button" onClick={onClose} className="absolute inset-0 bg-zinc-900/60 dark:bg-black/80 backdrop-blur-sm animate-in fade-in duration-200 border-0 cursor-default" aria-label="Close modal" />
...
const JobCountdown = ({ nextExecution }: { nextExecution: string | null }) => {
  ...
  useEffect(() => {
    ...
    const calculateTimeLeft = () => {
      const difference = +new Date(nextExecution) - Date.now();
      if (difference <= 0) {
        setTimeLeft('Running...');
        return;
      }
    ...
    if (loading && jobs.length === 0) {
      return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 animate-pulse">
          {['job-skeleton-1', 'job-skeleton-2', 'job-skeleton-3'].map((key) => (
            <div key={key} className="h-20 bg-zinc-50 dark:bg-zinc-850 border border-zinc-200/50 dark:border-zinc-800 rounded-xl" />
          ))}
        </div>
      );
    }
...
const KpiCardsSection = ({ displaySummary }: { displaySummary: any }) => {
  return (
    <div id="kpi-cards" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div className="group relative overflow-hidden bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-5 rounded-xl shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
        <div className="flex items-center justify-between mb-4">
          <div className="w-8 h-8 rounded-lg bg-zinc-50 dark:bg-zinc-850 flex items-center justify-center text-zinc-400">
            <Receipt size={16} />
          </div>
      ...
      <div className="group relative overflow-hidden bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-5 rounded-xl shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
        <div className="flex items-center justify-between mb-4">
          <div className="w-8 h-8 rounded-lg bg-zinc-50 dark:bg-zinc-805 flex items-center justify-center text-zinc-450">
            <Activity size={16} />
          </div>
          <ArrowUpRight size={14} className="text-zinc-300 dark:text-zinc-600" />
        </div>
      ...
      <div className="group relative overflow-hidden bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-5 rounded-xl shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
        <div className="flex items-center justify-between mb-4">
          <div className="w-8 h-8 rounded-lg bg-zinc-50 dark:bg-zinc-850 flex items-center justify-center text-zinc-450">
            <Users size={16} />
          </div>
          <ArrowUpRight size={14} className="text-zinc-300 dark:text-zinc-600" />
        </div>
...
const DriversCardSection = ({ displaySummary }: { displaySummary: any }) => {
  return (
    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 shadow-sm space-y-4">
...
const CustomerRevenueShareCard = ({
  customerPieData,
  summaryTypeFilter,
  displaySummary,
  isDark
  ...
  customerPieData: any[];
  summaryTypeFilter: string;
  displaySummary: any;
  isDark: boolean;
}) => {
  if (customerPieData.length === 0) return null;
  return (
...
const TopCustomersCard = ({
  ...
  customerRankData: any[];
  summaryTypeFilter: string;
  displaySummary: any;
  isDark: boolean;
}) => {
  if (customerRankData.length === 0) return null;
  return (
    <div className="min-h-[230px] rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/30 dark:bg-zinc-950/10 p-4">
      <div className="mb-3">
...
const ProductRevenueMixCard = ({
  ...
  if (productPieData.length === 0) return null;
  return (
    <div className="min-h-[230px] rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/30 dark:bg-zinc-950/10 p-4">
      <div className="mb-3">
...
const TopItemsSoldCard = ({
  ...
  if (productRankData.length === 0) return null;
  return (
    <div className="min-h-[230px] rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/30 dark:bg-zinc-950/10 p-4">
      <div className="mb-3">
...
const DetailedInsightsSection = ({
  ...
  if (customerRankData.length === 0 && productRankData.length === 0) return null;
    ...
    <div id="detailed-insights" className="space-y-3.5 animate-in fade-in duration-300">
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm">
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <CustomerRevenueShareCard
            customerPieData={customerPieData}
            summaryTypeFilter={summaryTypeFilter}
...
const HistoryLedgerSection = ({
  historyList,
  setSelectedSummary
}: {
  historyList: SalesSummary[];
  setSelectedSummary: (item: SalesSummary) => void;
}) => {
  return (
                ...
                >
                  ...
                  <td className="px-5 py-4 font-bold">
                    <span className={cn(
                      ...
                      getSummaryTypeBadgeClass(item.summaryType)
                    )}>
                      {item.summaryType}
                    </span>
                  </td>
...
const SummariesHeaderSection = ({
  activeSummary,
  isCreateAllowed,
  setIsGenModalOpen
}: {
  activeSummary: SalesSummary | null;
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-center justify-between bg-white dark:bg-zinc-900 p-6 border border-zinc-200 dark:border-zinc-700/80 rounded-xl shadow-sm gap-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        {activeSummary && (
          <div className="flex items-center gap-2 px-3 py-1.5 bg-zinc-50 dark:bg-zinc-800/40 border border-zinc-200/60 dark:border-zinc-800 rounded-xl text-[9px] font-bold uppercase tracking-wider text-zinc-650 dark:text-zinc-450 shadow-sm animate-in fade-in duration-200">
...
const TelemetryChartSection = ({
  ...
  telemetryChartData: TelemetryPoint[];
  isFetching: boolean;
  isDark: boolean;
  summaryTypeFilter: string;
  setSummaryTypeFilter: (t: string) => void;
  periodStartDate: string;
  setPeriodStartDate: (d: string) => void;
  periodEndDate: string;
  setPeriodEndDate: (d: string) => void;
  loadActiveSummary: () => void;
}) => {
  return (
    <div id="telemetry-charts" className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm flex flex-col h-[480px] relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-zinc-100 dark:border-zinc-800/60">
        <div className="flex items-center gap-2.5">
          <div className="w-1.5 h-4 bg-zinc-900 dark:bg-zinc-100 rounded-full" />
          <div>
            <h4 className="text-[10px] font-bold text-zinc-900 dark:text-zinc-100 uppercase tracking-widest flex items-center gap-1.5">
      ...
      {telemetryChartData.length === 0 ? (
        ...
        <div className="flex-1 h-full w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart
              key={`area-chart-${summaryTypeFilter}`}
...
const AIChatPanelSection = ({
  ...
  handleSendChatMessage: (customMessage?: string) => void;
}) => {
  return (
    <div className="lg:col-span-5 flex flex-col h-[480px] lg:h-[750px] bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden shadow-sm">
      ...
      <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-zinc-200 dark:scrollbar-thumb-zinc-800 animate-in fade-in duration-200">
        {chatMessages.map((msg, index) => {
          ...
          return (
            <div
              key={`msg-${index}-${msg.content.substring(0, 15)}`}
              className={cn(
                "flex flex-col max-w-[90%] rounded-2xl px-4 py-3 text-xs leading-relaxed shadow-sm font-medium",
                isModel
                  ? "bg-zinc-50 dark:bg-zinc-850/50 border border-zinc-200/65 dark:border-zinc-800 text-zinc-750 dark:text-zinc-300 self-start rounded-tl-none"
        ...
        {chatLoading && (
          <div className="flex items-center gap-1.5 self-start bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200/60 dark:border-zinc-850 px-4 py-3 rounded-2xl rounded-tl-none text-zinc-455">
...
export default function SalesSummariesPage() {
  ...
  const customerPieData = React.useMemo(() => {
    return customerRankData.slice(0, 6).map((item, index) => ({
      ...item,
  ...
  const productPieData = React.useMemo(() => {
    return productRankData.slice(0, 6).map((item, index) => ({
      ...item,
      fill: CHART_COLORS[index % CHART_COLORS.length]
    }));
  }, [productRankData]);
  ...
  return (
    <div className="space-y-6 w-full animate-in fade-in duration-300 pb-20">
      ...
      <div id="scheduler-jobs">
        <SchedulerJobsWidget jobs={jobsStatus} loading={loadingJobs} onRefresh={loadSchedulerStatus} />
      </div>

      {loading ? (
        <div className="space-y-6 animate-pulse">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {['card-skeleton-1', 'card-skeleton-2', 'card-skeleton-3', 'card-skeleton-4'].map((key) => (
              <div key={key} className="h-28 bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl" />
            ))}
          </div>
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
          ...
          <KpiCardsSection displaySummary={displaySummary} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
            ...
            <div className={cn("lg:col-span-7 flex flex-col gap-6 transition-opacity duration-200", isFetching && "opacity-75")}>
              ...
              <TelemetryChartSection
                ...
                isDark={isDark}
                summaryTypeFilter={summaryTypeFilter}
                setSummaryTypeFilter={setSummaryTypeFilter}
                periodStartDate={periodStartDate}
                setPeriodStartDate={setPeriodStartDate}
                periodEndDate={periodEndDate}
                setPeriodEndDate={setPeriodEndDate}
                loadActiveSummary={loadActiveSummary}
              />

              {/* Drivers Card */}
              <DriversCardSection displaySummary={displaySummary} />
            </div>
