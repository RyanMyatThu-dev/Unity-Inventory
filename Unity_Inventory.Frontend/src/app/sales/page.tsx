...
const SaleRow = memo(({ report, index, onSelect }: { 
  ...
  <tr 
    ...
    className="group hover:bg-zinc-50/50 transition-colors border-b border-zinc-50 last:border-0 cursor-pointer"
  >
    <td className="px-4 py-3 text-[10px] font-black text-zinc-400 w-12 text-center">
      {index}
    </td>
    ...
    <td className="px-4 py-3 text-right">
      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button 
          ...
          className="p-1.5 hover:bg-zinc-100 rounded text-zinc-400 hover:text-zinc-900 transition-colors"
        >
          <Eye size={14} />
        </button>
...
const SaleCard = memo(({ report, index, onSelect }: { 
  ...
  <div 
    onClick={() => onSelect(report.id)}
    className="group bg-white border border-zinc-100 rounded-xl p-5 hover:shadow-md transition-all relative overflow-hidden flex flex-col h-full cursor-pointer"
  >
    <div className="absolute top-2 right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
      <button 
        onClick={(e) => { e.stopPropagation(); onSelect(report.id); }}
        className="p-1.5 bg-white/80 backdrop-blur-sm border border-zinc-100 hover:bg-zinc-100 rounded shadow-sm text-zinc-400 hover:text-zinc-900 transition-colors"
      >
        <Eye size={12} />
      </button>
      <button className="p-1.5 bg-white/80 backdrop-blur-sm border border-zinc-100 hover:bg-zinc-100 rounded shadow-sm text-zinc-400 hover:text-zinc-900 transition-colors">
        <Download size={12} />
      </button>
    </div>
    
    <div className="flex items-center justify-between mb-4">
      <div className="w-6 h-6 bg-zinc-900/5 rounded-full flex items-center justify-center text-[10px] font-black text-zinc-400">
        {index}
      </div>
      <div className="px-2 py-0.5 bg-emerald-50 text-emerald-700 text-[9px] font-black uppercase rounded tracking-widest">
    ...
    <div className="flex items-center gap-3 mb-4">
      ...
      <div className="min-w-0">
        <p className="text-[11px] font-black text-zinc-900 truncate tracking-tight uppercase">INV-{report.id.toString().padStart(6, '0')}</p>
    ...
    <div className="mb-4 flex-1">
      <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-1">Customer Entity</p>
      <p className="text-[11px] font-bold text-zinc-700 uppercase line-clamp-1">{report.customerName || 'Walk-in Customer'}</p>
    </div>
    
    <div className="pt-4 border-t border-zinc-50 mt-auto">
      <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-1">Total Amount</p>
      <p className="text-lg font-black text-zinc-900 tracking-tighter">{formatCurrency(report.totalAmount)}</p>
    </div>
  </div>
));
...
const SaleDetailModal = ({ reportId, onClose }: { reportId: number, onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-2xl rounded-2xl shadow-2xl animate-in zoom-in-95 duration-200 overflow-hidden flex flex-col max-h-[90vh]">
        
        {/* Header */}
        <div className="p-6 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/50">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-zinc-900 rounded-xl flex items-center justify-center shadow-lg shadow-zinc-200">
              <FileText className="text-white" size={24} />
            </div>
            <div>
              <h3 className="text-sm font-black text-zinc-900 uppercase tracking-tighter">Invoice Details</h3>
              <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Invoice #{report.id.toString().padStart(6, '0')}</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-white rounded-full border border-transparent hover:border-zinc-100 transition-all shadow-sm">
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8">
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="space-y-1">
              ...
              <p className="text-xs font-black text-zinc-900 truncate uppercase">{report.customerName || 'Standard Client'}</p>
            </div>
            <div className="space-y-1 text-center">
              <div className="flex items-center justify-center gap-1.5 text-[9px] font-black text-zinc-400 uppercase tracking-widest">
                <Calendar size={10} /> Date
              </div>
              <p className="text-xs font-black text-zinc-900 uppercase">{new Date(report.reportDate).toLocaleDateString()}</p>
            </div>
            <div className="space-y-1 text-right">
              <div className="flex items-center justify-end gap-1.5 text-[9px] font-black text-zinc-400 uppercase tracking-widest">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Invoice Items</h4>
            </div>
            <div className="border border-zinc-100 rounded-xl overflow-hidden shadow-sm">
              <table className="w-full text-left border-collapse">
                <thead className="bg-zinc-50/50 border-b border-zinc-100">
                  <tr>
                    <th className="px-4 py-3 text-[9px] font-black text-zinc-400 uppercase tracking-widest">Description</th>
                    <th className="px-4 py-3 text-[9px] font-black text-zinc-400 uppercase tracking-widest text-center">Qty</th>
                    <th className="px-4 py-3 text-[9px] font-black text-zinc-400 uppercase tracking-widest text-right">Unit Price</th>
                    <th className="px-4 py-3 text-[9px] font-black text-zinc-400 uppercase tracking-widest text-right">Line Total</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-50">
                  {report.vouchers?.map((v, i) => (
                    <tr key={i} className="hover:bg-zinc-50/30 transition-colors">
                      <td className="px-4 py-3">
                        <p className="text-[11px] font-bold text-zinc-900 uppercase">{v.inventoryName}</p>
                        <p className="text-[9px] text-zinc-400 font-medium">Product ID: {v.id}</p>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span className="px-2 py-1 bg-zinc-100 rounded text-[10px] font-black text-zinc-600">{v.quantity}</span>
                      </td>
                      <td className="px-4 py-3 text-right text-[10px] font-bold text-zinc-500">
                        {(v.sellPrice || 0).toLocaleString()}
                      </td>
                      <td className="px-4 py-3 text-right text-[10px] font-black text-zinc-900">
            ...
            <div className="flex flex-col items-end gap-2 pt-4 border-t-2 border-zinc-900">
               <div className="flex items-center gap-8">
                  ...
                  <p className="text-2xl font-black text-zinc-900 tracking-tighter">{formatCurrency(report.totalAmount)}</p>
               </div>
               <div className="flex gap-2">
                  <button className="flex items-center gap-2 px-4 py-2 bg-zinc-900 text-white rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-zinc-800 transition-all shadow-md">
           ...
           <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="space-y-3">
                 ...
                 <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2 p-1 border border-zinc-100 rounded bg-white shadow-inner min-h-[100px]">
                    ...
                    {customerTotalPages > 1 && (
                      <div className="flex items-center justify-center gap-2 pt-1">
                        <button 
                          ...
                          className="p-1 border border-zinc-100 rounded hover:bg-zinc-50 disabled:opacity-20 transition-all"
                        >
                          <ChevronLeft size={12} />
                        </button>
                        <span className="text-[10px] font-black text-zinc-400">{customerPage} / {customerTotalPages}</span>
                        <button 
              ...
              <div className="space-y-3">
                 ...
                 <div className="grid grid-cols-2 gap-3 min-h-[200px]">
                    {inventories.map(inv => (
                      <button 
                        key={inv.id} 
                        onClick={() => addItem(inv)}
                        className="p-3 bg-white border border-zinc-100 rounded-lg flex items-center gap-3 hover:border-zinc-900 hover:shadow-md transition-all text-left group"
                      >
                         <div className="w-12 h-12 rounded bg-zinc-50 border border-zinc-100 flex items-center justify-center overflow-hidden shrink-0">
                            {inv.imageUrl ? (
                         ...
                         <div className="min-w-0 flex-1">
                            <p className="text-[11px] font-black text-zinc-900 uppercase tracking-tight truncate">{inv.name}</p>
                            <p className="text-[10px] font-bold text-emerald-600">{formatCurrency(inv.price)}</p>
                            <p className="text-[9px] text-zinc-400 font-medium italic">Stock: {inv.currentStock} units</p>
                         </div>
                      </button>
                    ))}
                 </div>
                 {productTotalPages > 1 && (
                   <div className="flex items-center justify-center gap-2 pt-2">
                     ...
                     <span className="text-[10px] font-black text-zinc-400">{productPage} / {productTotalPages}</span>
                     <button 
              ...
              <div className="flex-1 space-y-4 overflow-y-auto mb-6 pr-2 scrollbar-thin scrollbar-thumb-zinc-200">
                 {items.length > 0 ? items.map(item => (
                   <div key={item.inventoryId} className="flex items-center justify-between group animate-in slide-in-from-right-2 duration-150">
                      ...
                      <div className="flex items-center gap-2">
                         <p className="text-[10px] font-black text-zinc-900">{((item.price || 0) * item.quantity).toLocaleString()}</p>
  ...
  return (
    <div className="space-y-6 max-w-[1400px] animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-1 bg-white border border-zinc-100 p-1 rounded-lg shadow-sm">
          <button 
            onClick={() => setViewType('table')}
            className={cn(
              "p-1.5 rounded transition-all",
              viewType === 'table' ? "bg-zinc-900 text-white" : "text-zinc-400 hover:text-zinc-900 hover:bg-zinc-50"
            )}
          >
            <LayoutList size={14} />
          </button>
          <button 
            ...
            className={cn(
              "p-1.5 rounded transition-all",
              viewType === 'grid' ? "bg-zinc-900 text-white" : "text-zinc-400 hover:text-zinc-900 hover:bg-zinc-50"
            )}
          >
            <LayoutGrid size={14} />
          </button>
      ...
      {viewType === 'table' ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-zinc-100 bg-zinc-50/50">
                  <th className="px-4 py-3 text-[10px] font-black text-zinc-400 uppercase tracking-widest w-12 text-center">#</th>
              ...
              <tbody>
                {loading && reports.length === 0 ? (
                  [...Array(10)].map((_, i) => <tr key={i} className="animate-pulse border-b border-zinc-50"><td colSpan={5} className="h-14 bg-white" /></tr>)
                ) : reports.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
           {loading && reports.length === 0 ? (
             [...Array(8)].map((_, i) => <div key={i} className="h-48 bg-zinc-50 rounded-xl animate-pulse" />)
           ) : reports.length === 0 ? (
      ...
      {!loading && reports.length > 0 && (
        <div className="px-4 py-3 flex items-center justify-between border-t border-zinc-100 bg-zinc-50/30 rounded-lg mt-6 border border-zinc-100 shadow-sm">
