...
import { PlusCircle, LogOut, Loader2, ArrowRight } from 'lucide-react';
...
export default function SelectBusinessPage() {
  const { user, switchBusiness, refreshUser, logout } = useAuth();
  const [isCreating, setIsCreating] = useState(false);
  const [newBusinessName, setNewBusinessName] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);

  const businesses = user?.businesses ?? [];
  const showCreate = user ? canProvisionNewBusiness(user.accountType) : false;
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-900/50 flex items-center justify-center p-8 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-2xl z-10 space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {businesses.length === 0 ? (
            <div className="sm:col-span-2 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 p-10 text-center space-y-4">
              <p className="text-sm font-medium text-zinc-600 dark:text-zinc-300">
                {showCreate
                  ? 'You have no workspaces yet. Create one to get started.'
                  : 'You have not been assigned to any workspace. Contact an organization owner.'}
              </p>
              {showCreate && (
                <button
                  type="button"
                  onClick={() => setIsCreating(true)}
                  className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 text-xs font-bold uppercase tracking-widest hover:opacity-90 transition-opacity"
                >
                  <PlusCircle size={18} />
                  New workspace
                </button>
            ...
            <>
              ...
              {showCreate && (
                ...
                >
                  ...
                  <p className="text-[10px] font-bold text-zinc-400 group-hover:text-zinc-900 dark:text-zinc-100 uppercase tracking-widest">
                    New Workspace
                  </p>
                </button>
              )}
      ...
      {showCreate && isCreating && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="absolute inset-0 bg-zinc-900/60 backdrop-blur-sm" onClick={() => setIsCreating(false)} />
            ...
            <form onSubmit={handleCreateBusiness} className="p-8 space-y-8">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full px-5 py-4 rounded-xl bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 focus:bg-white dark:focus:bg-zinc-800 focus:border-zinc-900 dark:focus:border-zinc-500 outline-none transition-all text-xs font-semibold text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-300 dark:placeholder:text-zinc-600 shadow-inner"
