...
const CustomerRow = memo(({ customer, index, onSelect, onDelete }: { 
  ...
  >
    <td className="px-6 py-4 text-[10px] font-black text-zinc-400 w-12 text-center">
      {index}
    </td>
...
const CustomerCard = memo(({ customer, index, onSelect, onDelete }: { 
  ...
  <div 
    onClick={() => onSelect(customer)}
    className="group bg-white border border-zinc-100 rounded-xl p-6 hover:shadow-md transition-all cursor-pointer relative overflow-hidden flex flex-col h-full"
  >
    ...
    <div className="flex items-center gap-4 mb-6 relative">
      <div className="absolute -top-4 -left-4 w-6 h-6 bg-zinc-900/5 rounded-full flex items-center justify-center text-[10px] font-black text-zinc-400 shadow-sm">
        {index}
      </div>
      <div className="w-14 h-14 rounded-full bg-zinc-900 flex items-center justify-center text-white text-lg font-black shadow-lg shadow-zinc-200 shrink-0">
        {customer.name?.[0] || '?'}
      </div>
      <div className="min-w-0">
        <p className="text-sm font-black text-zinc-900 truncate uppercase tracking-tight">{customer.name}</p>
        <p className="text-[10px] text-zinc-400 font-bold italic tracking-wider">UID-{customer.id.toString().padStart(5, '0')}</p>
      </div>
    </div>
    ...
    <div className="pt-4 border-t border-zinc-50 mt-auto">
      <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-1">Lifetime Value</p>
      <p className="text-lg font-black text-zinc-900 tracking-tighter">{formatCurrency(customer.totalPurchased)}</p>
    </div>
  </div>
));
    ...
    <div className="space-y-6 max-w-[1400px] animate-in fade-in duration-300">
      ...
      {viewType === 'table' ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-zinc-50 bg-zinc-50/30">
                  <th className="px-6 py-4 text-[10px] font-black text-zinc-400 uppercase tracking-widest w-12 text-center">#</th>
              ...
              <tbody>
                {loading && customers.length === 0 ? (
                  [...Array(10)].map((_, i) => <tr key={i} className="animate-pulse border-b border-zinc-50"><td colSpan={6} className="h-16 bg-white" /></tr>)
                ) : customers.length === 0 ? (
                  <tr><td colSpan={6} className="px-6 py-32 text-center text-[10px] font-black text-zinc-300 uppercase tracking-widest italic">No client records found</td></tr>
                ) : (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-6">
           {loading && customers.length === 0 ? (
             [...Array(8)].map((_, i) => <div key={i} className="h-64 bg-zinc-50 rounded-xl animate-pulse" />)
           ) : customers.length === 0 ? (
             <div className="col-span-full py-32 text-center text-[10px] font-black text-zinc-300 uppercase tracking-widest italic">No client records found</div>
           ) : (
             customers.map((customer, idx) => (
               <CustomerCard 
                 key={customer.id} 
        ...
        {!loading && customers.length > 0 && (
        <div className="px-6 py-4 flex items-center justify-between border-t border-zinc-50 bg-zinc-50/20 rounded-xl mt-6 border border-zinc-100 shadow-sm">
