...
const CategoryNode = ({ 
  ...
  return (
    <div className="select-none">
      <div 
        className={cn(
          "group flex items-center justify-between py-2 px-3 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all cursor-pointer border border-transparent",
          level === 0 ? "mb-1" : "ml-4"
        )}
      >
        <div className="flex items-center gap-2 flex-1" onClick={() => setIsExpanded(!isExpanded)}>
          <div className="flex items-center justify-center w-5 h-5">
            {hasChildren ? (
              ...
              <div className="w-1 h-1 bg-zinc-300 dark:bg-zinc-700 rounded-full ml-1.5" />
            )}
          </div>
          <Folder size={16} className={cn("shrink-0", hasChildren ? "text-amber-500" : "text-zinc-400")} />
          <span className={cn("text-xs font-semibold tracking-tight", level === 0 ? "text-zinc-900 dark:text-zinc-100" : "text-zinc-600 dark:text-zinc-400")}>
            {category.categoryName}
          </span>
          {category.description && (
            <span className="text-[10px] text-zinc-400 font-medium truncate max-w-[200px] hidden md:block opacity-60">
              — {category.description}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button 
            ...
            className="p-1.5 hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors"
          >
            <Plus size={12} />
          </button>
          <button 
          ...
          <button 
            ...
            className="p-1.5 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded text-zinc-400 hover:text-rose-600 transition-colors"
          >
            <Trash2 size={12} />
          </button>
        </div>
      ...
      {hasChildren && isExpanded && (
        <div className="border-l border-zinc-200 dark:border-zinc-800 ml-5 mt-1">
...
export default function CategoriesPage() {
  ...
  return (
    <div className="max-w-[1000px] animate-in fade-in duration-300">
      <div className="flex items-center justify-between bg-white dark:bg-zinc-900 p-6 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-sm mb-6">
        <div>
          <h1 className="text-base font-bold text-zinc-900 dark:text-zinc-100 uppercase tracking-widest flex items-center gap-2">
            <LayoutList size={18} />
            Category Architecture
          </h1>
          <p className="text-[10px] text-zinc-400 font-semibold uppercase tracking-wider mt-1">Hierarchical taxonomy & inventory grouping.</p>
        </div>
        <button 
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-sm overflow-hidden p-6 min-h-[400px]">
        {loading && tree.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            <Loader2 size={24} className="animate-spin text-zinc-300" />
            <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Loading Taxonomy...</p>
          </div>
        ) : tree.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 text-center">
            <div className="w-12 h-12 rounded-full bg-zinc-50 dark:bg-zinc-800/50 flex items-center justify-center mb-4">
               <Folder size={20} className="text-zinc-300" />
            </div>
            <p className="text-[10px] font-bold text-zinc-300 uppercase tracking-widest italic">No categories defined yet</p>
            <button onClick={() => handleOpenCreate()} className="mt-4 text-[10px] font-bold text-zinc-900 dark:text-zinc-100 underline decoration-2 underline-offset-4">Create your first category</button>
          </div>
        ) : (
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 w-full max-w-sm rounded-2xl shadow-2xl p-8 animate-in zoom-in-98 duration-150">
            <div className="flex items-center gap-2 mb-8 border-b border-zinc-50 dark:border-zinc-800 pb-4">
               <div className="w-7 h-7 bg-zinc-100 dark:bg-zinc-800 rounded-lg flex items-center justify-center">
                  {editingCategory ? <Edit2 size={12} className="text-zinc-600" /> : <Plus size={12} className="text-zinc-600" />}
               </div>
               <div>
                  <h3 className="text-[10px] font-bold text-zinc-900 dark:text-zinc-100 uppercase tracking-widest">
                    {editingCategory ? 'Modify Category' : parentCategory ? 'Add Sub-category' : 'New Root Category'}
                  </h3>
                  {parentCategory && (
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                <label className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest px-1">Label</label>
                <input 
                  required 
                  autoFocus
                  type="text" 
                  ...
                  className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 rounded-xl text-xs font-semibold text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-300 dark:placeholder:text-zinc-600 outline-none focus:ring-1 focus:ring-zinc-900 dark:focus:ring-zinc-400" 
                />
              </div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                >
                  {isSubmitting ? <Loader2 size={14} className="animate-spin m-auto" /> : editingCategory ? 'Update record' : 'Confirm Registration'}
                </button>
                <button 
                  type="button"
